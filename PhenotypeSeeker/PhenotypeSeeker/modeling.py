...
class Input():
    ...
    def get_input_data(cls, inputfilename, take_logs, mpheno):
        ...
        with open(inputfilename) as inputfile:
            header = inputfile.readline().split() #['ID', 'Addresses', 'Azithromycin']
    ...
    def _get_phenotypes_to_analyse(cls, mpheno):
        ...
        for item in cls.mpheno_to_index:
            ...
            # print(cls.phenotypes_to_analyse)
...
class Samples():
    ...
    def get_kmer_lists(self):
        ...
        # print(self.kmer_length,self.cutoff)
        process = run(
...
class phenotypes():
    ...
    def kmer_testing_setup(cls):
        ...
        ps.wait()
        # print('cls.no_kmers_to_analyse.value',cls.no_kmers_to_analyse.value)
    ...
    def get_kmers_tested(self, split_of_kmer_lists):
        ...
        for line in zip(*[open(item) for item in split_of_kmer_lists]):
            ...
            if counter%self.progress_checkpoint.value == 0:
                Input.lock.acquire()
                # print('cls.no_kmers_to_analyse.value', self.no_kmers_to_analyse.value)# 3673716
                # print(self.progress_checkpoint.value)#4953
                # print(counter)
        ...
        # print(counter%self.progress_checkpoint.value)
        # print('counter',counter)
        stderr_print.currentKmerNum.value += counter%self.progress_checkpoint.value#counter/4953
        # print(stderr_print.currentKmerNum.value)
        Input.lock.release()
        stderr_print.check_progress(
            self.no_kmers_to_analyse.value, "tests conducted.", self.name + ": "
...
def modeling(args):
    ...
    if not Input.jump_to:
        ...
        list(map(
            lambda x:  x.test_kmers_association_with_phenotype(), #test_kmers_association_with_phenotype : start from here!
