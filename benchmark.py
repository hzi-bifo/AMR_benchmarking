...
def combine_data(species,antibiotics,fscore, f_phylotree, f_kma,tool_list):
    # 'Point-/ResFinder', 'Neural networks', 'AMRP', 'Seq2Geno2Pheno','PhenotypeSeeker', 'Kover'
...
def extract_info(level,s, fscore, cv_number, f_phylotree, f_kma,f_all):
    if fscore=='f1_macro' or fscore=='accuracy':
        tool_list=['Point-/ResFinder', 'Neural networks', 'AMRP', 'Seq2Geno2Pheno','PhenotypeSeeker', 'Kover','Majority']
    else:
        tool_list=['Point-/ResFinder', 'Neural networks', 'AMRP', 'Seq2Geno2Pheno','PhenotypeSeeker', 'Kover']
    ...
    fig.subplots_adjust(wspace=0.25, hspace=0.3, top=0.9, bottom=0.05)
    ...
    for species, antibiotics_selected in zip(df_species_radar, antibiotics_radar):
        ...
        axs_.set_title(species, weight='bold',style='italic', size='medium', position=(0.5, 1.1),
    ...
    if fscore=='f1_macro' or fscore=='accuracy':
        ...
        axs_.legend(handles=legend_elements,  labels=['', '','', '', '','', '']+labels,ncol=2, handlelength=3, edgecolor='grey',
          borderpad=0.7, handletextpad=1.5, columnspacing=0,labelspacing=1,loc='lower right',title='Software')
    else:
        legend_elements = p_radar+[
                               Patch(facecolor=colors[0],edgecolor= colors[0]),Patch(facecolor=colors[1],edgecolor= colors[1]),
                               Patch(facecolor=colors[2],edgecolor= colors[2]),Patch(facecolor=colors[3],edgecolor= colors[3]),
    ...
    for species, antibiotics_selected in zip(df_species_bar, antibiotics_bar):
        ...
        g.set_title(species,style='italic', weight='bold')
