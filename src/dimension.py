...
def extract_info(level,s, fscore, cv_number, f_phylotree, f_kma,f_all):
    ...
    df_plot = pd.DataFrame(columns=['species_k_cutting','k-mers'])
        ...
        if k==31 or k==13:
            for species, antibiotics in zip(df_species, antibiotics):
                ...
                for anti in antibiotics:
                    id=ID[i_anti]
                    i_anti+=1
                    for canonical in [True,False]:#todo False
                        voca=[]
                        if canonical:
                            kmer_temp='./patric_2022/K-mer_lists/cano'+str(k)+'mer'
                        else:

                            kmer_temp='./patric_2022/K-mer_lists/non_cano'+str(k)+'mer'
                        for i in id:
                        ...
                        df_plot = df_plot.append(df_plot_sub, sort=False)
    print(df_plot)
    df_plot['k-mers'] = df_plot['k-mers'].astype(int)
    sns.set_theme(style="whitegrid")
    plt.figure(figsize = (10,30))
    ax = sns.violinplot(x='k-mers', y='species_k_cutting', data=df_plot,inner=None, color=".8")
    ax = sns.stripplot(x='k-mers', y='species_k_cutting', data=df_plot,edgecolor='#373737')
    plt.axvline(1000)
    ...
    fig = ax.get_figure()
    fig.savefig("log/results/kmer_dimensions.png")
