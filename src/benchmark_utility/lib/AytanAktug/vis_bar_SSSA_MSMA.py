...
def combinedata(species,level,df_anti,merge_name,fscore,learning,epochs,f_fixed_threshold,f_nn_base,f_optimize_score,temp_path,output_path):
    ...
    for each_anti in antibiotics:
        ...
        summary_plot_single.loc['e'] = [single_results.loc[species,each_anti ], each_anti, 'single-species-antibiotic model',single_results_std.loc[species,each_anti]]
        ...
        summary_plot_dis.loc['e'] = [dis_results.loc[species,each_anti ], each_anti, 'Discrete databases multi-species model',np.nan]
