...
def extract_info(level,s, fscore,f_all,f_step,f_mean_std,output_path):
    ...
    if f_step=='1':
        ...
        fig.subplots_adjust(wspace=0.1, hspace=0.25, top=0.95, bottom=0.02,left=0.05)
        # fig.suptitle('A', weight='bold',size=35,x=0.02,y=0.97 )
        [axi.set_axis_off() for axi in axs.ravel()[11:]]
        i=0
        for species, antibiotics_selected in zip(df_species, antibiotics):
            ...
            for col_t in df:
                ...
                if col==0:
                    ...
                    ax.set(ylabel=None)
                    ax.tick_params(axis='y',bottom=False)
                if species=='Neisseria gonorrhoeae' and ( i_color in [1,2,3]):
                    if f_mean_std=='mean':
                        leg=ax.legend(bbox_to_anchor=(2.2, 0.6),ncol=1,fontsize=30,frameon=False, markerscale=2)
                    else:
                        leg=ax.legend(bbox_to_anchor=(4, 1.38),ncol=1,fontsize=30,frameon=False, markerscale=2)
        ...
        # -----------------------------
        if f_mean_std=='mean':
            fig.savefig(output_path+'Results/final_figures_tables/F6_RobustBySpecies_mean.png')
            fig.savefig(output_path+'Results/final_figures_tables/F6_RobustBySpecies_mean.pdf')
        elif f_mean_std=='std':
            fig.savefig(output_path+'Results/supplement_figures_tables/S2_RobustBySpecies_std.png')
            fig.savefig(output_path+'Results/supplement_figures_tables/S2_RobustBySpecies_std.pdf')
        else:
        ...
        [axi.set_axis_off() for axi in axs.ravel()[5:]]
        i=0
        for tool in tool_list:
            ...
            if f_mean_std=='mean':
                ...
                ax.set(ylim=(0, 0.5))
            if col==0:
                ax.set_ylabel(fscore.replace("_", "-").capitalize(),size = 35)
            ...
            ax.set_title(tool, weight='bold',size=38)
            ax.set(xticklabels=[])
            ax.set(xlabel=None)
            ax.tick_params(axis='x',bottom=False)
            # if i==12:
        ...
        # -----------------------------
        if f_mean_std=='mean':
            fig.savefig(output_path+'Results/final_figures_tables/F6_RobustByTool_mean.png')
            fig.savefig(output_path+'Results/final_figures_tables/F6_RobustByTool_mean.pdf')
            ...
            fig.savefig(output_path+'Results/supplement_figures_tables/S2_RobustTool_std.png')
            fig.savefig(output_path+'Results/supplement_figures_tables/S2_RobustTool_std.pdf')
        else:
            print('Wrong parameters set, please check.')
            exit(1)
