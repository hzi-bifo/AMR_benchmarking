...
def extract_info(level,s, fscore, f_all):
    ...
    df1.to_excel("log/results/cv_results.xlsx", sheet_name='introduction')
    ...
    for species, antibiotics_selected in zip(df_species, antibiotics):
        ...
        wb = load_workbook('log/results/cv_results.xlsx')
        ew = pd.ExcelWriter('log/results/cv_results.xlsx')
        ew.book = wb
