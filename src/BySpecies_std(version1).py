...
def ComBySpecies(level,s, fscore, cv_number, f_phylotree, f_kma,f_all):
    ...
    fig.subplots_adjust(wspace=0, hspace=0.2, top=0.95, bottom=0.03)
    if f_phylotree:
    ...
    for species, antibiotics_selected in zip(df_species_radar, antibiotics_radar):
        ...
        for d, color in zip(data, colors):
            ...
            p_,=axs_std.plot(theta_, d,  'o-', markersize=4,color=color,dashes=[6, 2],linewidth=2)
        ...
        axs_std.set(xticklabels=[])
        axs_std.set(xlabel=None)
        axs_std.tick_params(axis='x',bottom=False)
        ...
        axs_ = inset_axes(axs_std,
                    ...
                    bbox_to_anchor=(0,-0.45),    # data coordinates
        ...
        if species=='Streptococcus pneumoniae':
            axs_.tick_params(axis='x', which='major', color='grey',grid_linestyle='dashed', pad=10,labelsize=16,zorder=3)
        else:
            axs_.tick_params(axis='x', which='major', color='grey',grid_linestyle='dashed', pad=15,labelsize=16,zorder=3)
        ...
        adjust_lable(axs_,antibiotics,anti_share,colors_anti)
