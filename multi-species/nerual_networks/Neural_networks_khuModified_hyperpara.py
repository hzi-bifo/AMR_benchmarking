...
def eval(species, antibiotics, level, xdata, ydata, p_names, p_clusters, cv, Random_State,
    ...
    return score
#
# def multiAnti(species, antibiotics, level, xdata, ydata, p_names, p_clusters, cv, Random_State,
#          re_epochs, f_scaler,f_fixed_threshold,f_no_early_stop,f_kma,f_tree, f_optimize_score, save_name_score,concat_merge_name,threshold_point,min_cov_point):
#     '''Normal CV, only for multi-Anti model.'''
#
#     # data
#     data_x = np.loadtxt(xdata, dtype="float")
#     data_y = np.loadtxt(ydata)
...
#     folders_sample,folders_sample_test=neural_networks.cluster_folders.prepare_folders_multiAnti(cv,species,antibiotics,p_names,f_kma,f_tree,level)#todo
#
#     hyper_space = hyper_range(anti_number, f_no_early_stop, antibiotics)
#
#
...
#     test_samples = folders_sample_test[out_cv]
#     x_test = data_x[test_samples]
#     y_test = data_y[test_samples]
#     x_test = torch.from_numpy(x_test).float()
#     y_test = torch.from_numpy(y_test).float()
