...
#            echo " glistquery ${sample}  -l ${feature_path}/${species}/${anti}_temp/${anti}_feature_vector_13_union.list "
        ...
        python ./AMR_software/PhenotypeSeeker/multiSpecies/get_weights_multi.py -s ${species} -cv ${j} -anti ${anti} -temp ${log_path}
        #4. perform chi-square test,and filter the kmers that :N_samples_w_kmer <  2 or N_samples_wo_kmer < 2
        python ./AMR_software/PhenotypeSeeker/multiSpecies/chisquare_multi.py -s ${species} -cv ${j} -anti ${anti} -temp ${log_path}
        #5. filter the kmers according to the p-value
        python ./AMR_software/PhenotypeSeeker/multiSpecies/filter_multi.py -s ${species} -cv ${j} -anti ${anti} -temp ${log_path}

        # 2).testing------------------
        # no need to get weights

        python ./AMR_software/PhenotypeSeeker/multiSpecies/filter_multi.py -s ${species} -cv ${j} -f_test -anti ${anti} -temp ${log_path}
