...
def main():
    ...
    if 'kma_indexing' in out_lst:
        out_lst.remove('kma_indexing')
    ...
    for species in out_lst:
        ...
        cmd1 = 'cp ./db_pointfinder/'+str(species.replace(" ", "_"))+'/resistens-overview.txt %s' % (path_merge+'/temp/'+str(species.replace(" ", "_")))
        # cmd2 = 'cp ./db_pointfinder/'+str(species.replace(" ", "_"))+'/resistens-overview.txt %s' % (path_merge+'/temp/'+str(species.replace(" ", "_"))+'_temp')
        ...
        with open(resistens_file_summary, 'a') as outfile:
            with open(path_merge+'/temp/'+str(species.replace(" ", "_"))) as infile:
                ...
                if i==0:
                    ...
                    print(line_list[1].split('\t')[2])
                    for line in line_list[1:]:
                        id = line.split('\t')[0]
                        outfile.write(id + "_" + species)
                        outfile.write("\t")
