#!/bin/bash
#SBATCH --job-name=8LOO  # Name of job
...
#SBATCH --mem=80G # Memory per node | Alternative: --mem-per-cpu
...
##########################################################################
#preapre concatenated reference database for Point-/ResFinder
...
#echo "Finished: use Point-/ResFinder to extract SNPs and genes." #finished. Sep 8th.
...
#for i in {8..8};do
#echo ${i}
#python ./AMR_software/AytanAktug/main_MSMA_concat_backupSep24th.py -f_nn -i_CV ${i} -cv 5 -f_all -f_kma -f_optimize_score 'f1_macro' -f_fixed_threshold -learning 0.0 -e 0 -temp ${log_path} -l ${QC_criteria} || { echo "Errors in Aytan-Aktug leave-one(specie)-out MSMA concatenated databases model. Exit ."; exit; }
#done
##
