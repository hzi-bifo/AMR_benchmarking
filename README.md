...
- Memory requirment: Some procedures require extremely large memory. Aytan-Aktug multi-species model (adaption version) feature building needs up to 370G memory, ML evaluation needs up to 80G memory. Some other procedures requiring large memory are KMA clustering in Aytan-Aktug SSSA (on the magnitude of  of 10G, depending on the number of CPU set). TODO
...
- The script `main.sh` goes through the whole benchmarking process from data reprocessing to running software, to basic benchmarking results visualization. 
- But you can't finishe this AMR benchamrking study by one click fr several reasons. You may need manually install Seq2Geno if errors occur and then you have to access Geno2Pheno website using the feature generated by Seq2Geno. Due to large data sets and time consuming ML model learning process, altogether may take more than 2 months with 20 CPUs accompanied by 10 GPUs, so it's not realistic to warp all benchmarking scripts in a pipeline.


### B.  <a name="workflows"></a>Workflow of usage in steps

You can refer to `main.sh` and `./scripts/model/<software_name>.sh` for details.
...
- Some tips for running. SSSA model support both CPU parallelization running and GPU sequential running. (via gpu_on in Config.yaml). Other 4 multi- models are designed for GPU running and sequential multi-tasks due to heavy computing load. 4.4 and 4.5 are based on some intermediate results from 4.3, so please run the 3 multi-species  models sequentially.
