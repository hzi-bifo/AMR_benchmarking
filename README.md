...
- We suggest you to run `main.sh` step by step by commenting some codes there at one time. `./scripts/model/<software_name>.sh` also provides more detailed instructions of running this benchmarking workflow. You can't finish the whole AMR benchamrking just by submitting `main.sh` to run once and for all due to several reasons. You have to access Geno2Pheno website using the feature generated by Seq2Geno. Due to large data sets and time consuming ML model learning process, which altogether may take more than 2 months with 20 CPUs accompanied by 10 GPUs, you may need to run different tasks on different machines and re-run some processes if it accidently terminates unexpectedly during a long periord of time.
