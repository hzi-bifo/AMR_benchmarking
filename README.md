...
|log_path| To where to generate the `log` folder for the tempary files, which you can delete by hand afterwards. (note:some software will generate large amount of temp files to 60G (TODO check!), if you run all species in parallel.) Large temp files are stored under `/log/software/<software_name>/software_output` | ./|
