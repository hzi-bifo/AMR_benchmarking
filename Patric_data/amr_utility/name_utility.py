...
def GETname_multi_bench_save_name_final(species,level,learning,epochs,f_fixed_threshold,f_nn_base,f_optimize_score):
    if f_optimize_score=='auc':
        save_name_score_f = str(level) + '/' + str(species.replace(" ", "_")) + '/' + 'lr_' + str(
            learning) + '_ep_' + str(epochs) + '_base_' + str(f_nn_base) + '_ops_' + f_optimize_score
    else:
...
def GETname_multi_bench_multi(level,path_large_temp,merge_name):
    ...
    run_file_roary1=run_file_roary='./cv_folders/' + str(level) + '/multi_species/' +merge_name+ "_roary1.sh"
    run_file_roary2 = run_file_roary = './cv_folders/' + str(level) + '/multi_species/' + merge_name + "_roary2.sh"
    run_file_roary3 = run_file_roary = './cv_folders/' + str(level) + '/multi_species/' + merge_name + "_roary3.sh"
...
def GETname_multi_bench_multi_species(level,path_large_temp,merge_name,s):
    ...
    path_res_result = '/net/sgi/metagenomics/data/khu/benchmarking/resfinder_results/' + str(s.replace(" ", "_"))
    # -----
